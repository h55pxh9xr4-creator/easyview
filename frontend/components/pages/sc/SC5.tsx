...
import {
  fetchSC5Summary, fetchSC5Extract, fetchVCHVoucherDetail,
  SC5Summary, SC5Extract, VCHCounterLineItem,
} from "@/lib/api";
...
export default function SC5() {
  ...
  const [summary,   setSummary]   = useState<SC5Summary[] | null>(null);
  const [loading,   setLoading]   = useState(false);
  const [selDate,   setSelDate]   = useState<string | null>(null);
  const [extract,   setExtract]   = useState<SC5Extract[] | null>(null);
  ...
  const load = () => {
    ...
    fetchSC5Summary(dateFrom, dateTo)
  ...
  const handleSelectDate = async (date: string) => {
    ...
    setExLoading(true);
    try {
      setExtract(await fetchSC5Extract(dateFrom, dateTo, date));
  ...
  const totalExpense = summary ? summary.reduce((s, r) => s + r.total_expense, 0) : 0;
  const totalCash    = summary ? summary.reduce((s, r) => s + r.total_cash, 0) : 0;
  const uniqueDays   = summary ? summary.length : 0;
  const kpiCnt       = selRow ? selRow.cnt           : totalCnt;
  const kpiExpense   = selRow ? selRow.total_expense  : totalExpense;
  const kpiCash      = selRow ? selRow.total_cash     : totalCash;
  ...
  const chartData = {
    ...
    datasets: [{
      data: summary?.map(r => r.total_cash) ?? [],
      borderColor: "rgba(147,51,234,1)",
      backgroundColor: summary?.map(r => r.date === selDate ? "rgba(147,51,234,1)" : "rgba(147,51,234,0.35)") ?? [],
      ...
      tooltip: {
        ...
        callbacks: {
          label: (ctx: { parsed: { y: number }; dataIndex: number }) =>
            summary
              ? ` 현금 ${fmtAmt(ctx.parsed.y)}${unitLabel} · 비용 ${fmtAmt(summary[ctx.dataIndex].total_expense)}${unitLabel} · ${summary[ctx.dataIndex].cnt}건`
              : "",
        },
      },
    },
  ...
  return (
    <div className="wrap">
      <div className="info-note">
        💡 예상위험: 중당금을 계상하지 않고 비용 인식과 동시에 현금지급, 발생주의 위배 가능성
      </div>
      ...
      {summary && (
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 14 }}>
          <KpiCard label="탐지 전표 수" value={`${fmtN(kpiCnt)}건`} sub={kpiSub} active={!!selDate} isDark={isDark} />
          <KpiCard label="현금지급 합계" value={`${fmtAmt(kpiCash)}${unitLabel}`} sub={kpiSub} active={!!selDate} isDark={isDark} color={RED} />
          <KpiCard label="비용인식 합계" value={`${fmtAmt(kpiExpense)}${unitLabel}`} sub={kpiSub} active={!!selDate} isDark={isDark} color={BLUE} />
          <KpiCard label={selDate ? "선택 날짜" : "탐지 일수"} value={selDate ? selDate : `${fmtN(uniqueDays)}일`} sub={kpiSub} active={!!selDate} isDark={isDark} />
        </div>
      )}
                ...
                <table style={{ tableLayout: "fixed", width: "100%" }}>
                  ...
                  <thead style={{ position: "sticky", top: 0, background: theadBg, zIndex: 1 }}>
                    <tr>
                      ...
                      <th style={{ textAlign: "center", color: BLUE }}>비용인식</th>
                      <th style={{ textAlign: "center", color: RED }}>현금지급</th>
                    </tr>
                  </thead>
                  <tbody>
                    {extract.map((r, i) => {
                      ...
                      return (
                        <tr key={i} onClick={(e) => { if (isSel) { const rect = e.currentTarget.getBoundingClientRect(); triggerComment({ page: PAGE, label: r.voucher_no, value: `${fmtN(r.cash_amount)}원` }, { top: rect.top, right: rect.right }, e.currentTarget); } else { handleSelectVch(r.voucher_no); } }}
                          ...
                          <td style={{ textAlign: "right", color: BLUE, fontWeight: 600, whiteSpace: "nowrap" }}>
                            {r.expense_amount ? fmtN(r.expense_amount) : "-"}
                          </td>
                          <td style={{ textAlign: "right", color: RED, fontWeight: 600, whiteSpace: "nowrap" }}>
                            {r.cash_amount ? fmtN(r.cash_amount) : "-"}
                          </td>
                        </tr>
                      );
                    })}
                  ...
                  <tfoot>
                    <tr style={{ fontWeight: 700, background: tfootBg }}>
                      ...
                      <td style={{ textAlign: "right", color: BLUE, whiteSpace: "nowrap" }}>
                        {fmtN(extract.reduce((s, r) => s + r.expense_amount, 0))}
                      </td>
                      <td style={{ textAlign: "right", color: RED, whiteSpace: "nowrap" }}>
                        {fmtN(extract.reduce((s, r) => s + r.cash_amount, 0))}
                      </td>
                    </tr>
                  </tfoot>
                </table>
          ...
          <div style={{ maxHeight: 300, overflowY: "auto", overflowX: "auto" }}>
            {lnLoading ? (
              ...
              <table>
                ...
                <tbody>
                  {lines.map((v, i) => {
                    const isCash    = v.dr_cr === "대변";
                    const isExpense = v.dr_cr === "차변";
                    return (
                      <tr key={i} onClick={(e) => { const rect = e.currentTarget.getBoundingClientRect(); triggerComment({ page: PAGE, label: v.voucher_no, value: `${fmtN(v.amount)}원`, bodyTemplate: `페이지: ${PAGE}\n일자: ${v.date}\n전표번호: ${v.voucher_no}\n계정과목: ${v.account_name}\n거래처: ${v.counterparty}\n적요: ${v.description}\n금액: ${fmtN(v.amount)}원\n\n문의내용:\n` }, { top: rect.top, right: rect.right }, e.currentTarget); }} style={{ cursor: "pointer" }}>
                        ...
                        <td style={{ textAlign: "center" }}>
                          {isCash ? (
                            <span style={{ fontSize: 11, fontWeight: 700, color: RED, background: "rgba(220,38,38,0.08)", borderRadius: 10, padding: "2px 8px" }}>→ 현금지급</span>
                          ) : isExpense ? (
                            <span style={{ fontSize: 11, fontWeight: 700, color: BLUE, background: "rgba(37,99,235,0.08)", borderRadius: 10, padding: "2px 8px" }}>● 비용인식</span>
                          ) : null}
                        </td>
                        ...
                        <td style={{ textAlign: "right", color: isExpense ? BLUE : zeroClr, whiteSpace: "nowrap" }}>
                          {isExpense ? fmtN(v.amount) : "-"}
                        </td>
                        <td style={{ textAlign: "right", color: isCash ? RED : zeroClr, whiteSpace: "nowrap" }}>
                          {isCash ? fmtN(v.amount) : "-"}
                        </td>
                      </tr>
                    );
                  })}
