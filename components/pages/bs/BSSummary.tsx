    ...
    <div className="wrap">
      ...
      {[
        ...
        <div key={cat} style={{ display: "grid", gridTemplateColumns: "420px 1fr", gap: 14, alignItems: "stretch" }}>
          {kpiLoading || !kpiForIdx ? <KpiSkeleton /> : <div style={{ cursor: "pointer", position: "relative", ...lift(cat) }} onClick={(e) => { const r = e.currentTarget.getBoundingClientRect(); triggerComment({ page: "BS 요약", label: cat, value: `${fmtB(kpiForIdx.ending)}백만` }, { top: r.top, right: r.right }); }}>{ck.has(commentKey("BS 요약", cat)) && <CommentDot inquiryId={ck.get(commentKey("BS 요약", cat))!} />}<BSKpiCard cat={cat} data={kpiForIdx} selectedLabel={selectedLabel} current={currentVal} noncurrent={noncurrentVal} /></div>}
          <div className="card">
      ...
      <div>
        ...
        {actLoading ? (
          ...
          <div key={item.label} style={{ display: "grid", gridTemplateColumns: "420px 1fr", gap: 14, alignItems: "stretch", marginBottom: 14 }}>
            {/* 왼쪽 카드 */}
            <div className="card" style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", cursor: "pointer", position: "relative", ...lift(item.label) }} onClick={(e) => { const r = e.currentTarget.getBoundingClientRect(); triggerComment({ page: "BS 요약", label: item.label, value: `${days.toFixed(1)}일` }, { top: r.top, right: r.right }); }}>
