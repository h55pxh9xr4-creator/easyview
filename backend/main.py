...
_VIEW_JE = """
    ...
    WHERE j.report_id = (
        ...
        WHERE r.is_active = 1
        AND (
            COALESCE((SELECT value FROM view_settings WHERE key='view_company'), '') = ''
            OR r.company = COALESCE((SELECT value FROM view_settings WHERE key='view_company'), '')
        )
...
with engine.connect() as conn:
    ...
    if "je" in _tables:
        conn.execute(text("DROP TABLE je")); conn.commit()
    if "tb_account" in _tables:
        conn.execute(text("DROP TABLE tb_account")); conn.commit()

    # view_settings 테이블 (없으면 생성)
    conn.execute(text("""
    ...
    """))
    conn.execute(text(
        "INSERT OR IGNORE INTO view_settings (key, value) VALUES ('view_company', '')"
    ))
    conn.commit()

    # 뷰 항상 DROP → 재생성 (SQL 변경 반영)
    conn.execute(text("DROP VIEW IF EXISTS je"))
    conn.execute(text("DROP VIEW IF EXISTS tb_account"))
    conn.commit()
    conn.execute(text(_VIEW_JE))
    conn.execute(text(_VIEW_TB))
    conn.commit()
