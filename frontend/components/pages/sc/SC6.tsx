                ...
                <table style={{ tableLayout: "fixed", width: "100%" }}>
                  ...
                  <tbody>
                    {extract.map((r, i) => {
                      ...
                      return (
                        <tr key={i} onClick={(e) => { if (isSel) { const rect = e.currentTarget.getBoundingClientRect(); triggerComment({ page: PAGE, label: r.voucher_no, value: `${fmtN(r.dr_amount || r.cr_amount)}원` }, { top: rect.top, right: rect.right }); } else { handleSelectVch(r.voucher_no); } }}
                          style={{ cursor: "pointer", background: isSel ? selRowBg : undefined, borderLeft: isSel ? `3px solid ${ORANGE}` : "3px solid transparent" }}>
