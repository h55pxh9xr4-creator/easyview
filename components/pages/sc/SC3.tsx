    ...
    <div className="wrap">
      ...
      {summary && (
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          ...
          <KpiCard label="대변 합계" value={`${fmtM(kpiAmt)}원`} sub={kpiSub} active={!!selDate} />
          <KpiCard label={selDate ? "선택 날짜" : "주말 일수"} value={selDate ? selDate : `${fmtN(kpiDays)}일`} sub={kpiSub} active={!!selDate} />
        </div>
      )}

      {/* ── 차트 + 추출 내역 (항상 2-col, CSS transition으로 열림) ── */}
      <div style={{
        display: "grid",
        ...
        {/* 전표 추출 내역 — 항상 DOM에 있음, 0px일 때 overflow:hidden으로 숨겨짐 */}
...
function KpiCard({ label, value, sub, active }: { label: string; value: string; sub?: string; active?: boolean }) {
  return (
    <div className="card" style={{ padding: "12px 20px", minWidth: 0, flex: 1, borderTop: active ? "3px solid #E87722" : "3px solid transparent", transition: "border-color 0.2s" }}>
      ...
      {sub && <div style={{ fontSize: 10, color: active ? "#E87722" : "#bbb", marginTop: 4, fontWeight: active ? 600 : 400 }}>{sub}</div>}
    </div>
  );
}
