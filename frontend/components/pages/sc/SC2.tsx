      ...
      <div style={{ display: "grid", gridTemplateColumns: selected ? "1fr 1fr" : "1fr", gap: 14, marginBottom: 14 }}>
        <div className="card">
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            ...
            {exceptions && <span style={{ fontSize: 12, color: dimTxt }}>{fmtN(exceptions.length)}건</span>}
            {!selected && exceptions && exceptions.length > 0 && (
              <span style={{ fontSize: 11, color: dimTxt, marginLeft: "auto" }}>👆 행 클릭 → 상세 확인</span>
            )}
            {selected && (
          ...
          <div style={{ height: 360, overflowY: "auto" }}>
            {loading ? (
              <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, color: dimTxt }}><Spinner /> 조회 중...</div>
        ...
        {selected && (
          <div className="card">
            ...
            <div style={{ height: 360, overflowY: "auto" }}>
              {extract === null ? (
                <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, color: dimTxt }}><Spinner /> 로딩 중...</div>
              ) : (
                <table>
                  <thead style={{ position: "sticky", top: 0, background: theadBg, zIndex: 1 }}>
                    <tr>
                  ...
                  <tbody>
                    {extract.map((r, i) => (
                      <tr key={i} onClick={(e) => { const rect = e.currentTarget.getBoundingClientRect(); triggerComment({ page: PAGE, label: r.voucher_no, value: `${fmtN(r.amount)}원` }, { top: rect.top, right: rect.right }, e.currentTarget); }} style={{ cursor: "pointer" }}>
                        ...
                        <td style={{ textAlign: "center", color: subTxt, fontSize: 11 }}>
                          {r.voucher_no}
