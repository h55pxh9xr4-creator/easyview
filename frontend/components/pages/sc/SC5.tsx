    ...
    <div className="wrap">
      ...
      {summary && (
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 14 }}>
          <KpiCard label="탐지 전표 수" value={`${fmtN(kpiCnt)}건`} sub={kpiSub} active={!!selDate} />
          <KpiCard label="현금지급 합계" value={fmtM(kpiCash)} sub={kpiSub} active={!!selDate} color={RED} />
          <KpiCard label="비용인식 합계" value={fmtM(kpiExpense)} sub={kpiSub} active={!!selDate} color={BLUE} />
          <KpiCard label={selDate ? "선택 날짜" : "탐지 일수"} value={selDate ? selDate : `${fmtN(uniqueDays)}일`} sub={kpiSub} active={!!selDate} />
        </div>
      )}
                  ...
                  {lines.map((v, i) => {
                    ...
                    return (
                      <tr key={i} onClick={(e) => { const rect = e.currentTarget.getBoundingClientRect(); triggerComment({ page: PAGE, label: v.voucher_no, value: `${fmtN(v.amount)}원`, bodyTemplate: `페이지: ${PAGE}\n일자: ${v.date}\n전표번호: ${v.voucher_no}\n계정과목: ${v.account_name}\n거래처: ${v.counterparty}\n적요: ${v.description}\n금액: ${fmtN(v.amount)}원\n\n문의내용:\n` }, { top: rect.top, right: rect.right }); }} style={{ cursor: "pointer" }}>
                        ...
                        <td style={{ textAlign: "right", color: isExpense ? BLUE : "#DDD", whiteSpace: "nowrap" }}>
                          {isExpense ? fmtN(v.amount) : "-"}
                        </td>
                        <td style={{ textAlign: "right", color: isCash ? RED : "#DDD", whiteSpace: "nowrap" }}>
